...
## load seqinfo with offline fallback
...
loadSeqInfoWithFallback <- function(genomeBuild, genomeStyle) {
  tryCatch({
    getSeqInfo(genomeBuild, genomeStyle)
  }, error = function(e) {
    warning(paste0(
      "getSeqInfo() failed (", e$message, ") -- attempting local chrom sizes fallback"
    ))

    scriptDir <- getScriptDir()
    candidateFiles <- c(
      file.path(scriptDir, paste0(genomeBuild, ".chrom.sizes")),
      file.path(getwd(), paste0(genomeBuild, ".chrom.sizes")),
      "/home/gissu/ichorCNA/hg38.chrom.sizes"
    )
    candidateFiles <- unique(candidateFiles[file.exists(candidateFiles)])

    if (length(candidateFiles) == 0) {
      stop(paste0(
        "Could not build seqinfo: UCSC download failed and no local ",
        genomeBuild,
        ".chrom.sizes file was found"
      ))
    }

    chromFile <- candidateFiles[1]
    message("Using local chrom sizes file for seqinfo: ", chromFile)

    chromSizes <- read.table(
      chromFile,
      header = FALSE,
      sep = "\t",
    ...
    if (ncol(chromSizes) < 2) {
      stop(paste0("Invalid chrom sizes file format: ", chromFile))
    }

    seqNames <- as.character(chromSizes[[1]])
    seqLens <- suppressWarnings(as.numeric(chromSizes[[2]]))
    keep <- !is.na(seqNames) & seqNames != "" & !is.na(seqLens)
    seqNames <- seqNames[keep]
    seqLens <- seqLens[keep]

    if (tolower(genomeStyle) == "ncbi") {
      seqNames <- sub("^chr", "", seqNames, ignore.case = TRUE)
    } else if (tolower(genomeStyle) == "ucsc") {
      needsPrefix <- !grepl("^chr", seqNames, ignore.case = TRUE)
      seqNames[needsPrefix] <- paste0("chr", seqNames[needsPrefix])
    }
...
seqinfo <- loadSeqInfoWithFallback(genomeBuild, genomeStyle)
...
for (n in normal){
  for (p in ploidy){
    ...
    mainName[counter] <- paste0(id, ", p: ", p, ", log likelihood: ", signif(hmmResults.cor$results$loglik[hmmResults.cor$results$iter], digits = 4))
