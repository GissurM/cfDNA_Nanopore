...
# Comprehensive cfDNA Fragmentomics Analysis Pipeline - SOFT CLIP REQUIRED VERSION
# Processing all 24 T2T-aligned BAM files for CCCA motif analysis
# This version REQUIRES soft clips (keeps only 80% of reads where Dorado trimming worked)
...
RESULTS_DIR="$SCRIPT_DIR/results2_softclip"
...
for BAM_FILE in "$BAM_DIR"/*_aligned.bam; do
    ...
    if [ -f "$MOTIF_CSV_FILE" ]; then
        ...
        echo "Step 2: Counting motifs (SOFT CLIP REQUIRED)..."
        Rscript $SCRIPT_DIR/count_motif_softclip_required.R \
            "$CHR_LIST" \
            "$OUTPUT_DIR/" \
            "$OUTPUT_DIR/" \
            "$RESULTS_DIR/" \
