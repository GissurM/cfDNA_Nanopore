...
#### keeping reads mapped on selected chromosomes, with MAPQ > 20, readlength < 700
#### REQUIRES soft clipping (s1 > 0 for forward, s2 > 0 for reverse) to ensure proper trimming
stats <- stats[V8 > 20 & V1 %in% chr[,1] & V11 < 700 & V11 > 50 & 
               ((V6 == "+" & V10 > 0) | (V6 == "-" & V13 > 0))]
debug <- c(debug, paste("Stats filtered reads (with soft clip requirement):", nrow(stats)))
