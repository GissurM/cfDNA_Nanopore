###
# This is a version of the deconvolve.py script from the MethAtlas package that has been altered to match newer versions of packages. Additionally legend was changed to only include tissues that are present in the output graph.
...
def hide_small_tissues(df):
    ...
    df = pd.concat([df, others.rename('other').to_frame().T])
    return df
...
def plot_res(df, outpath, show=False):
    ...
    # Keep track of which tissues are actually visible (have non-zero values)
    legend_handles = []
    legend_labels = []
    
    for i in range(nr_tissues):
        tissue_values = list(df.iloc[i, :])
        tissue_max = max(tissue_values)
        
        # Only create bar and add to legend if tissue has visible contribution
        if tissue_max > 0:
            bar = plt.bar(r, tissue_values,
                    edgecolor='white',
                    width=0.85,
                    label=df.index[i],
                    bottom=bottom,
            ...
            # Add to legend only if visible
            legend_handles.append(bar)
            legend_labels.append(df.index[i])
        
        bottom += np.array(tissue_values)
    ...
    # Add vertical lines to separate sample groups: 1-5, 6-10, 11+
    for boundary in [5, 10]:
        if nr_samples > boundary:
            plt.axvline(x=boundary - 0.5, color='black', linewidth=3, alpha=0.8)

    # Add a legend with only visible tissues and a title
    plt.legend(legend_handles, legend_labels, loc='upper left', bbox_to_anchor=(1, 1), ncol=1)
